...
async function readCurlInput(): Promise<string> {
  ...
  return readInteractiveInput('Paste your cURL command from Care.com DevTools:');
}

async function handleParseCurl(options: any): Promise<void> {
  try {
    const curlInput = await readCurlInput();
...
async function handleStatus(): Promise<void> {
  ...
  const authAge = Date.now() - new Date(config.authenticatedAt).getTime();
  ...
  if (config.lastRefreshedAt) {
    ...
    const refreshMins = Math.floor(refreshAge / (1000 * 60));
    console.log(`  Last refresh: ${refreshMins}m ago`);
  }
...
async function handleSetDefaults(options: any): Promise<void> {
  const config = await loadConfig();
  if (!config) {
...
async function handlePing(options: any): Promise<void> {
  const config = await loadConfig();
  if (!config) {
    console.error('Not authenticated. Run: carecom auth parse-curl');
    process.exit(1);
